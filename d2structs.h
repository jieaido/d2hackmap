...
struct UnitAny {
	...
	DWORD	*pSeqMode;		//+30	(holds a pointer to skill sequence) 
	DWORD	dwSeqFrameCount;//+34	(frame * 256, count for sequence) 
	DWORD	dwSeqFrame;		//+38	(frame * 256 remaining for sequence) 
	DWORD	dwAnimSpeed;	//+3C	32Bit
	DWORD	dwSeqMode;		//+40	(holds current anim mode of sequence) 
	DWORD	dwCurrentFrame;	//+44	(frame * 256 remaining for animation)
	DWORD	dwFrameCount;	//+48	(frame * 256) 
	WORD	wAnimSpeed;		//+4C
	BYTE	nActionFrame;	//+4E
	BYTE	_unk;			//+4F	保证16位对齐
	void	*pAnimData;		//+50	ptr to the record for current *.COF file in AnimData.d2 
	void	*pGfxData;		//+54	ptr to GfxData struct 
	void	*pGfxDataExt;	//+58	another copy of pGfxData 
	StatList *pStatList;	//+5C	
	UnitInventory *pInventory; //+60
	void	*pLightMap;		//+64
	DWORD	dwLightRadius;	//+68
	WORD	wPl2ShiftIndex;	//+6C
	WORD	wUpdateType;	//+6E
	DWORD	dwUpdateUnit;	//+70	check if self have updated , 1 or 0 
	void	*pQuestRecord ;	//+74	for quest monsters (etc) 
	BYTE	nSparkyChest;	//+78	boolean used only for sparky chests, to get the extra drop
	BYTE	_1[3];			//+79
	void	*pTimerArgs;	//+7C	arguments to pass to timer 
	DWORD	dwSoundSync ;	//+80	used by summons and ambient stuff  serverside is pGame 
	DWORD	_2[4];			//+84
	...
	SkillInfo *pSkill;		//+A8
	void	*pCombat;		//+AC
	DWORD	dwHitClass;		//+B0	the hitclass for the last hit suffered 
	DWORD	_4[4];			//+B4
	DWORD	dwFlags1;		//+C4
	DWORD	dwFlags2;		//+C8
	DWORD	_5[6];			//+CC
...
struct DrlgAct {
	DWORD	dwFlag;			//+00	1
	DWORD	*pUnk1 ;		//+04	point to a struct 
	DrlgMisc *pDrlgMisc;	//+08
	DrlgRoom1 *pRoom1;		//+0C
	DWORD	_unk1;			//+10	0
	DWORD	dwActNo;		//+14
	void	*pfnCallBack;	//+18	a function pointer 
};
...
struct DrlgMisc {
	...
	void	*pfnCallBack;		//+80
	DrlgAct	*pDrlgAct;			//+84
	DWORD	dwBossTombLvl;		//+88  SuperUnique 古代无魂之卡(Ancient Kaathe Soulless)出现的地方
	DWORD	_2[3];				//+8C
	DWORD	*pMemPool;			//+98
	DWORD	_3[244];			//+9C
	DrlgLevel *pLevelFirst;		//+46C	未出过城时此列表并不能连接所有地图
	BYTE	_4[5];				//+470
	BYTE	nActNo;				//+475
	BYTE	_5[2];				//+476
	DWORD	dwStaffTombLvl;		//+478	插帐处
};

struct DrlgLevel {			//size = 0x230
	DWORD	dwflag;			//+00
	DWORD	dwPosX;			//+04
	DWORD	dwPosY;			//+08
	DWORD	dwSizeX;		//+0C
...
struct DrlgRoom2 {
	RoomTile *pRoomTiles;	//+00
	DWORD	_1[1];			//+04
	DWORD	dwPresetType;	//+08  1=随机生成  2=固定
	DWORD	_2;				//+0C
	DWORD	dwRoomsNear;	//+10
	DWORD	_3[2];			//+14
...
struct DrlgRoom1 {
	...
	DWORD	dwPosGameX;	//+08
	DWORD	dwPosGameY;	//+0C
	DWORD	dwSizeGameX;//+10
	DWORD	dwSizeGameY;//+14
	DWORD	dwPosRoomX;	//+18
	DWORD	dwPosRoomY;	//+1C
	DWORD	dwSizeRoomX;//+20
	DWORD	dwSizeRoomY;//+24
	DWORD	_1[3];		//+28
	...
	DrlgRoom1 *pNext;	// +74
	DWORD	_5;			//+78
	DWORD	dwRoomsNear;//+7C
};

struct CollMap {
    DWORD	dwPosGameX;	//+00
    DWORD	dwPosGameY;	//+04
    DWORD	dwSizeGameX;//+08
    DWORD	dwSizeGameY;//+0C
    DWORD	dwPosRoomX;	//+10
    DWORD	dwPosRoomY;	//+14
    DWORD	dwSizeRoomX;//+18
    DWORD	dwSizeRoomY;//+1C
    WORD	*pMapStart;	//+20
    WORD	*pMapEnd;	//+22
};
